  ...
  <div class="body1">
    ...
    <div class="content">
      <el-menu
        ...
        @select="handleSelect"
      >
        <el-menu-item index="1" style="font-size: 20px">大赛通知</el-menu-item>
        <el-menu-item index="2" style="font-size: 20px">新闻快讯</el-menu-item>
      </el-menu>
      ...
      <div class="list_item" v-for="(item, index) in informList" :key="index">
        ...
        <span class="list_item_time">{{ item.uploadTime }}</span>
      </div>
    </div>
    <!-- 分页条 -->
    <div class="container">
...
import { queryPageApi } from "@/apis/annunciation.js";
import { queryNewsPageApi } from "@/apis/news.js";
...
const title = ref("大赛通知");
let cskeys = ref({ title: "", pageNo: 1, pageSize: 10 });
const informList = ref([]);

const search = async () => {
  let result;
  if (activeIndex.value == 1) {
    result = await queryPageApi(cskeys.value);
  } else {
    result = await queryNewsPageApi(cskeys.value);
  }
  if (result.code == 1) {
...
const clickItem = (id) => {
  router.push({
    path: "/informContent",
    query: { id: id, Type: activeIndex.value },
  });
};
</script>
